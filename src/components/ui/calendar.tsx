...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn("p-2 sm:p-3 rounded-xl bg-purple-950/30 backdrop-blur-sm border border-purple-800/30", className)}
      classNames={{
        ...
        nav_button: cn(
          ...
          "h-6 w-6 sm:h-7 sm:w-7 bg-purple-900/20 p-0 text-white/80 hover:bg-purple-800/40 hover:text-white transition-colors rounded-lg"
        ),
        nav_button_previous: "absolute left-1",
        nav_button_next: "absolute right-1",
        table: "w-full border-collapse space-y-1",
        head_row: "grid grid-cols-7",
        head_cell: "text-purple-300 w-7 sm:w-9 font-medium text-[0.7rem] sm:text-[0.8rem] uppercase tracking-wider text-center mb-2",
        ...
        day_today: 
          "bg-purple-800/30 text-white ring-2 ring-purple-400/30",
      ...
      components={{
        IconLeft: ({ ...props }) => <ChevronLeft className="h-3 w-3 sm:h-4 sm:w-4 transition-transform duration-200 hover:scale-110" />,
        IconRight: ({ ...props }) => <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4 transition-transform duration-200 hover:scale-110" />,
      }}
      {...props}
    />
  )
